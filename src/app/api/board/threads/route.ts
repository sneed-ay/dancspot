...
// Column mapping:
// Frontend "category" <-> DB "dance_type"
// Frontend "title" <-> DB "nickname"
// Frontend "author" <-> DB "line_display_name"
...
export async function GET(request: NextRequest) {
  try {
    ...
    const threads = (data || []).map(toFrontend);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!postId || !lineUserId) {
      return NextResponse.json(
        { error: 'postId and lineUserId are required' },
        { status: 400 }
      );
    }
